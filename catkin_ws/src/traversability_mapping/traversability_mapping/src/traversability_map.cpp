...
public:
        ...
        , mapArrayCount(0)
    {
        ...
        // amcl_pose = nh.subscribe<geometry_msgs::PoseWithCovarianceStamped>("/amcl_pose", 1, &TraversabilityMapping::PoseHandler, this);
        gazebo_pose = nh.subscribe("/gazebo/model_states", 1, &TraversabilityMapping::PoseHandler, this);
    ...
    /*
    void PoseHandler(const geometry_msgs::PoseWithCovarianceStampedConstPtr& poseMsg){
        // Lock thread
        std::lock_guard<std::mutex> lock(mtx);
        // Get Robot Position
        robotPoint.x = poseMsg->pose.pose.position.x;
        robotPoint.y = poseMsg->pose.pose.position.y;
        robotPoint.z = poseMsg->pose.pose.position.z;
    }*/
    ...
    void initializeLocalOccupancyMap()
    {
        ...
        occupancyMap2DHeight.header.frame_id = "odom"; //odom
    ...
    bool getRobotPosition()
    {
        ...
        //     ROS_ERROR("Transform Error! Map -> getRobotPosition() %s", ex.what());
        //     ros::Duration(0.5).sleep();
        // }
        ...
        // robotPoint.x = amcl_pose.pose.position.x;
        // robotPoint.y = amcl_pose.pose.position.y;
        // robotPoint.z = amcl_pose.pose.position.z;
    ...
    void publishTraversabilityMap()
    {
        ...
        laserCloudTemp.header.frame_id = "odom"; //odom
