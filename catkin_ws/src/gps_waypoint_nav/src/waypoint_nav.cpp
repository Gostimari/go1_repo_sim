...
std::vector <std::pair<double, double>> getWaypoints(std::string path_local)
{
    ...
    for(int i = 0; i < numWaypoints; i++)
    {
        fileRead >> x;
        fileRead >> y;
        waypointVect.push_back(std::make_pair(x, y));
    }
    fileRead.close();
...
geometry_msgs::PointStamped latLongtoUTM(double lati_input, double longi_input)
{
    ...
    UTM_point_output.header.frame_id = "base";
...
geometry_msgs::PointStamped UTMtoMapPoint(geometry_msgs::PointStamped UTM_input)
{
    ...
    while(notDone)
    {
        try
        {
            ...
            listener.waitForTransform("odom", "base", time_now, ros::Duration(3.0));
    ...
    ROS_INFO("UTM to Map Point Conversion: x = %.2f, y = %.2f", map_point_output.point.x, map_point_output.point.y);
...
move_base_msgs::MoveBaseGoal buildGoal(geometry_msgs::PointStamped map_point, geometry_msgs::PointStamped map_next, bool last_point)
{
    ...
    ROS_INFO("Goal x: %.2f y: %.2f", goal.target_pose.pose.position.x, goal.target_pose.pose.position.y);
    return goal;
}

int main(int argc, char** argv)
{
    ros::init(argc, argv, "waypoint_nav"); //initiate node called gps_waypoint
    ros::NodeHandle n;
    ROS_INFO("Initiated waypoint_nav node");
    ...
    for(iter = waypointVect.begin(); iter < waypointVect.end(); iter++)
    {
        ...
        {
            ROS_ERROR("Go1 was unable to reach its goal. Waypoint unreachable.");
    ...
    ROS_INFO("Go1 has reached all of its goals!!!\n");
