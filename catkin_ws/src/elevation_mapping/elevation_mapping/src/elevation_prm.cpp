...
public:
    TraversabilityPRM():
        ...
        //subGoal = nh.subscribe<move_base_msgs::MoveBaseActionGoal>("/move_base/goal", 5, &TraversabilityPRM::goalPosHandler, this); // /prm_goal
        //subGoal = nh.subscribe<move_base_msgs::MoveBaseActionGoal>("/move_base_simple/goal", 5, &TraversabilityPRM::goalPosHandler, this);
        subGoal = nh.subscribe<geometry_msgs::PoseStamped>("/prm_goal", 5, &TraversabilityPRM::goalPosHandler, this); // /prm_goal
        subElevationMap = nh.subscribe<elevation_msgs::OccupancyElevation>("/elevation_map_fused_visualization/elevation_occupancy", 5, &TraversabilityPRM::elevationMapHandler, this);     
        //subElevationMap = nh.subscribe<nav_msgs::OccupancyGrid>("/elevation_map_fused_visualization/elevation_grid", 5, &TraversabilityPRM::elevationMapHandler, this);
        ...
        //pubGlobalPath = nh.advertise<nav_msgs::Path>("/move_base/GlobalPlanner/plan", 5); // /global_path // /move_base/DWAPlannerROS/global_plan

        gazebo_pose = nh.subscribe("/gazebo/model_states", 1, &TraversabilityPRM::PoseHandler, this);
    ...
    void elevationMapHandler(const elevation_msgs::OccupancyElevation::ConstPtr& mapMsg){
        ...
        ROS_INFO("updateMapBoundary");

        //updateCostMap();

        ROS_INFO("updateCostMap");

        buildRoadMap();

        ROS_INFO("buildRoadMap");
    }
    ...
    ////////////////////////////////////////////// Planner /////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /*void goalPosHandler(const move_base_msgs::MoveBaseActionGoalConstPtr& msg){
    	
        goalState->x[0] = msg->goal.target_pose.pose.position.x;
        goalState->x[1] = msg->goal.target_pose.pose.position.y;
        goalState->x[2] = msg->goal.target_pose.pose.position.z;
...
int main(int argc, char** argv){
    ...
    ros::init(argc, argv, "elevation_prm");
